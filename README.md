...
2. **今日简报** — 卡片流首页：洞察 / 决策 / 创作 / 待办 / 数据 / 提问六类卡片，
   支持 ← 跳过、↑ 收藏、→ 采纳（键盘 / 触摸 / 拖拽）；v0.7 起卡片内容可交互
   （选项可选、待办可勾、文本可编辑、卡片可跳转，交互态持久化），采纳时选中项自动注入任务目标；
   决策瞬间真实落章（双线印面 + 印泥纹理 + 砸下回震）；专注模式下采纳转入后台执行，
   全部批示完成后统一处理
...
v0.6.1 起界面字号支持 12–22px 拖拽连续调节且**全局生效**（根字号缩放，侧边栏/顶栏/底部导航等所有文字随动；≥18px 超大档自动隐藏次要信息，面向高龄用户）；
登录页为账号密码式正式页面，居中紧凑布局，背景动效呼应「卡片简报 + 工作台」产品形态（漂浮卡片 + 鼠标视差跟随 + 点击迸裂）；移动端底部导航含 iOS 安全区避让与输入防缩放。

## 质量保障

`npm test` 运行 Vitest 单测（147 例，覆盖 SSE 解析 / 增量归一化 / Markdown 渲染 / 工具注册 / 剧本与模板匹配 / 存储往返 / 偏好注入 / 导出工具 / CSV 成绩解析 / 数据源降级 / 上下文压缩 / 技能提炼与检索 / 简报卡校验与个性化排序 / 简报交互覆盖层 / 后台任务泵 / 数据上下文聚合与截断 / LLM 技能提炼校验与去重进化）；
...
v0.8.1 · 小型优化（专注模式移动端操作逻辑：后台执行指示器移动端收进顶栏不再遮挡卡片与落章、批示过程中点击指示器可随时查看后台任务明细浮层、批示完成总结层按钮移动端纵向堆叠；工作台多文档跳转：右栏文档面板新增上一份/下一份切换与 n/m 位置指示，点击标题展开全部文档清单直达切换并标注生成中文档，移动端文档抽屉内同样生效）
v0.8 · API 模式数据驱动闭环（数据上下文层：按角色聚合 SourceProvider 真实数据格式化为 prompt 文本（teacher 班级学情 / schoolAdmin 校情趋势与预警 / bureau 区域指标），来源标注 + 1800 字符截断护栏 + 单源失败不阻断；简报 API 数据驱动：user prompt 注入真实数据 + 「禁止编造」硬约束，无数据时定性描述，解析/校验失败自动重试一次后回退 Mock；文档生成数据注入：引用具体数字以数据上下文为准；LLM 技能提炼落地：trace 摘要 → LLM 复盘 → 候选校验 → 去重进化（触发词并集、版本+1、保留原步骤），任务完成后自动沉淀/进化技能并广播徽标，自进化闭环在 Mock 与 API 双模式均成立）
v0.7 · 智能简报（简报生成增强：Mock 剧本个性化排序（历史决策/收藏驱动）+ LLM 结构化生成骨架（逐卡运行时校验、超时与失败回退 Mock）；卡片内容可交互：选项可选/待办可勾/文本可编辑/卡片跳转，交互态 LocalStorage 持久化，采纳时选中项注入任务目标；真实落章动效：双线印面 + SVG 印泥纹理 + 砸下回震，仅滑卡决策瞬间；卡片动效：拖拽倾斜/拿起放大/阴影加深 + 堆叠视差联动 + 补位与入场 stagger；专注模式：采纳后台执行 + 浮动任务指示器 + 批示完成总结层一键统一处理，设置可关闭）
v0.6.1 · 小型优化（登录页改版为账号密码式正式页面，居中紧凑布局 + 卡片简报/工作台动效背景 + 鼠标视差/点击迸裂交互；界面字号 12–22px 拖拽连续调节且全局生效（根字号缩放覆盖全部 UI 文字）；移动端底部导航安全区避让修复输入框遮挡 + 简报 banner 精简）

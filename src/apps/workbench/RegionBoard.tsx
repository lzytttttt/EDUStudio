  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex items-center justify-between border-b border-line px-4 py-3">
        ...
        <span className="text-[0.625rem] text-ink-mute">12 所监测校</span>
      </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-2">
              {metrics.map((m, i) => {
                ...
                return (
                  ...
                  >
                    <p className="truncate text-[0.625rem] text-ink-mute" title={m.name}>{m.name}</p>
                    <div className="mt-1 flex items-baseline gap-1.5">
                      <span className={cn('text-lg font-semibold tabular-nums', active ? 'text-primary' : 'text-ink')}>
                        {m.value}
                      </span>
                      <span className={cn('inline-flex items-center gap-0.5 text-[0.625rem] font-medium', up ? 'text-mint' : 'text-danger')}>
            ...
            {metric && (
              <div className="mt-4 rounded-2xl border border-line bg-surface-2 p-3">
                <p className="mb-1 truncate text-[0.6875rem] font-medium text-ink-soft">{metric.name} · 近 6 期</p>
                ...
                <p className="mt-1 text-[0.625rem] leading-relaxed text-ink-mute">{metric.note}</p>
              </div>
            )}
